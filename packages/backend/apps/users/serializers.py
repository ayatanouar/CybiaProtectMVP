...
class UserSignupSerializer(serializers.ModelSerializer):
    ...
    def validate_email(self, value):
        allowed_emails = getattr(settings, "ALLOWED_OAUTH_EMAILS", [])
        if allowed_emails and value not in allowed_emails:
            raise serializers.ValidationError("Cette adresse e-mail n'est pas autorisée.")
        return value
...
class CookieTokenObtainPairSerializer(jwt_serializers.TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        allowed_emails = getattr(settings, "ALLOWED_OAUTH_EMAILS", [])
        if allowed_emails and self.user.email not in allowed_emails:
            raise exceptions.ValidationError("Connexion refusée : cet email n'est pas autorisé.")
